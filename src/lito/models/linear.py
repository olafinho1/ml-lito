...
class StackedLinearLayers(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            inplace:
                Whether to use inplace nonlinearity when possible
        ...
        if self.nonlinearity == "leaky_relu":
            nonlinearity_fun = lambda: nn.LeakyReLU(negative_slope=0.01, inplace=self.inplace)
        elif self.nonlinearity == "relu":
            nonlinearity_fun = lambda: nn.ReLU(inplace=self.inplace)
        elif self.nonlinearity == "tanh":
            nonlinearity_fun = nn.Tanh
        elif self.nonlinearity == "sigmoid":
            nonlinearity_fun = nn.Sigmoid
        elif self.nonlinearity in {"silu", "swish"}:
            nonlinearity_fun = lambda: nn.SiLU(inplace=self.inplace)
        elif self.nonlinearity == "gelu":
            nonlinearity_fun = nn.GELU
        else:
        ...
        for layer_idx in range(self.num_layers - 1):
            ...
            if self.dropout_prob > 0:
                additional_layers.append(nn.Dropout(p=self.dropout_prob, inplace=self.inplace))
